...
use embeddb_core::collection::{Collection, IndexType};
use embeddb_core::config::{CollectionConfig, SearchQuery};
...
use embeddb_index::{SearchResult, VectorIndex};
use rand::Rng;
    ...
    group.bench_function("insert_1000_persist", |b| {
        b.iter(|| {
            ...
            let pc = std::sync::Arc::new(
                embeddb_storage::page_cache::PageCache::open(&path, Default::default()).unwrap(),
            );
            ...
            for (i, v) in vectors.iter().enumerate() {
                col.insert(embeddb_core::config::Document::with_vector(
