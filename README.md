...
# 插入向量（384维）
vexra insert -c docs -v 0.1,0.2,...,0.384 -m '{"title":"入门指南"}'

# 用自然语言搜索
vexra search -c docs --text "如何开始" -k 5
```
...
cd embeddb
cargo build --release -p vexra-cli
...
```bash
vexra init                                    # 创建新数据库
vexra create-collection -n docs -d 3          # 创建3维集合
vexra insert -c docs -v 1.0,0.0,0.0          # 插入向量
vexra insert -c docs -v 0.0,1.0,0.0          # 再插一条
vexra search -c docs -v 1.0,0.1,0.3 -k 2     # 搜索最近邻
vexra serve                                    # 启动 Web 管理面板
```
...
```toml
[dependencies]
vexra-core = { git = "https://github.com/Mengxun326/embeddb" }
```
...
with embeddb.Database("data.vexra") as db:
...
const db = new Database('data.vexra');
...
在 Intel i7-13700H、AVX2 开启、128 维向量的环境下测试。完整基准：`cargo bench -p vexra-core`。
...
embeddb/
├── crates/
│   ├── vexra-core/        数据库、集合公共 API
│   ├── vexra-storage/     页格式、WAL、页缓存
│   ├── vexra-index/       HNSW、Flat、SIMD 距离计算
│   ├── vexra-metadata/    JSON 元数据、过滤器引擎
│   ├── vexra-query/       BM25、RRF 融合
│   ├── vexra-embedding/   文本向量化引擎
│   ├── vexra-ffi/         C ABI 多语言绑定基础
│   ├── vexra-cli/         命令行工具
│   └── vexra-server/       HTTP API + Dashboard
├── sdk/
